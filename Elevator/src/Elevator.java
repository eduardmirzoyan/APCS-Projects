...
public class Elevator extends JPanel implements ActionListener, Serializable {
	...
	public void setFloor(int f) {
		...
		direction[0] = !buttons[0] && (buttons[1] && !floorBits[1] || buttons[2] && !floorBits[1]
				|| buttons[2] && !floorBits[0] && !buttons[1]);
		direction[1] = (!buttons[0] && buttons[2] && !floorBits[1])
				|| (buttons[1] && !buttons[2] && floorBits[1] && floorBits[0])
				|| (!buttons[1] && buttons[2] && floorBits[1] && !floorBits[0])
				|| (buttons[0] && !buttons[1] && !buttons[2] && floorBits[1])
				|| (!buttons[0] && buttons[1] && !floorBits[1]);

		// Problem 6:
		lightUp = !floorBits[1] || !buttons[0] && buttons[2] && !floorBits[0];
		lightDown = !(!floorBits[1] || !buttons[0] && buttons[2] && !floorBits[0]);
