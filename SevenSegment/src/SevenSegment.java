...
public class SevenSegment extends JPanel implements ActionListener
{
  ...
  public void paintComponent(Graphics g)
  {
  	 ...
  	 segments[1] = !number[2] || (!number[1] && !number[0]) || (number[1] && number[0]);
  	 segments[2] = number[0] || number[2] || !number [1];
  	 segments[3] = number[3] || !number[2] && (!number[0] || number[1]) || number[1] && !number[0] || number[2] && number[0] && !number[1];
  	 segments[4] = (!number[2] && !number[0]) || (!number[3] && !number[0] && number[1]);
  	 segments[5] = (number[2] && !number[0]) || (!number[2] && number[3]) || (!number[3] && !number[1] && number[2]) || (!number[2] && !number[1] && !number[0]);
  	  //Mine
  	 segments[6] = number[3] || (number[1] && !number[2]) || (number[2] && !number[1]) || (number[2] && !number[0]);
